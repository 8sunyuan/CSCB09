...
void insert(int fd, int playing, char *ip)
{
    struct player *new;
...
int main(int argc, char **argv)
{
    ...
    while (!game_is_over()) {
        ...
        // Reads from stdin and broadcasts all players and their ip addresses
        if (FD_ISSET(STDIN_FILENO, &readfds)) {
            valread = read(STDIN_FILENO, command, 1024);
            strcpy(message, "");
            //printf("seems fine\n");
            for (p = playerlist; p ; p = p->next)
                sprintf(message, "%s%s has IP address %s\n", message, p->name, p->ip);
        ...
        if (FD_ISSET(server_socket, &readfds)) {
            ...
            insert(new_socket, 0, inet_ntoa(r.sin_addr));
            continue;
        }
